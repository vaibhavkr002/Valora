...
const server = http.createServer((req, res) => {
  ...
  try {
    ...
    // Test shop.html on desktop (1280px) and mobile (375px)
    for (const width of [375, 1280]) {
      ...
      const res = await send('Runtime.evaluate', {
        expression: `
          (function() {
            const sidebar = document.querySelector('.shop-sidebar, .shop-filter-sidebar');
            const filterBtn = document.querySelector('.btn-mobile-filter');
            return {
              page: 'shop.html',
              width: window.innerWidth,
              sidebarPosition: sidebar ? window.getComputedStyle(sidebar).position : null,
              filterBtnDisplay: filterBtn ? window.getComputedStyle(filterBtn).display : null
            };
          })()
        `,
        returnByValue: true
      });
      console.log(`Shop ${width}px:`, res.value);
    }

    // Test checkout.html on desktop (1280px) and mobile (375px)
    for (const width of [375, 1280]) {
      await send('Emulation.setDeviceMetricsOverride', { width, height: 900, deviceScaleFactor: 1, mobile: width < 768 });
      ...
      const res = await send('Runtime.evaluate', {
        expression: `
          (function() {
            const grid = document.querySelector('.checkout-grid, .checkout-main-grid');
            return {
              page: 'checkout.html',
              width: window.innerWidth,
              gridDisplay: grid ? window.getComputedStyle(grid).display : null,
              gridDirection: grid ? window.getComputedStyle(grid).flexDirection : null
            };
          })()
        `,
        returnByValue: true
      });
      console.log(`Checkout ${width}px:`, res.value);
    }
  } catch(e) {
    console.error(e);
  } finally {
